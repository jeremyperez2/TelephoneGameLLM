        ...
        for m_i, m in zip(plotter.model_indices, plotter.models):
            ...
            for p in plotter.prompts:
                ...
                for intial_story in plotter.stories:
                    print(key)
                    print(len(all_data["evolution"][p][intial_story][0][f"Results/{m}/{p}/{intial_story}"]))
...
with open("../Results/data-for-plotting-temperature/all_data.pkl", "rb") as file:
    all_data = pickle.load(file)


plotter = Plotter(all_data, models = ['Meta-Llama-3-8B-Instruct_Temp0', 'Meta-Llama-3-8B-Instruct_Temp0.4', 'Meta-Llama-3-8B-Instruct_Temp0.8', 'Meta-Llama-3-8B-Instruct_Temp1.2', 'Meta-Llama-3-8B-Instruct_Temp1.6'], prompts = ['rephrase', 'inspiration', 'continue'])
...
plotter.load_results(saving_name, all = False)
...
os.makedirs(f"Figures_temperature/{saving_name}", exist_ok=True)
...
_, _ = plotter.compare_temperature("position", ylim=None, fig=fig, axs=axs[0,:], legend_i=0, with_suptitle=True, legend_pos=(0.8, 0.8), labelpad = 5)
_, _ = plotter.compare_temperature("strength", ylim=None, fig=fig, axs=axs[1,:], legend_i=None, with_suptitle=False, labelpad = 40)
plt.savefig(f"Figures_temperature/{saving_name}/effect_of_temperature.pdf", bbox_inches="tight")
